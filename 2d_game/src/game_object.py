...
class GameObject():
    ...
    def get_center(self) -> Vector2D:
        return Vector2D(self.shape.x + self.shape.width / 2, self.shape.y + self.shape.height / 2)
    ...
    def update(self, delta_time: float):
        ...
        self.prev_y = self.shape.y
        self.shape.position = (
            self.shape.x + self.velocity.x * delta_time, self.shape.y + self.velocity.y * delta_time)
