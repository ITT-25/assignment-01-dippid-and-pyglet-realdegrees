    ...
    def update(self, delta_time: float):
        ...
        if self.state == GameState.INACTIVE:
            ...
            if self.player_1.is_ready() and self.player_2.is_ready():
                self.ball.set_velocity((self.paddle_left.get_center(
                ) - self.ball.get_center()).normalize() * INITIAL_BALL_SPEED)
            ...
            if self.reset_timer <= 0 and self.launch_timer > 0:
                self.reset()
                self.launch_timer -= delta_time
            elif self.launch_timer <= 0:
                self.ball.set_velocity((self.last_scorer.paddle.get_center(
                ) - self.ball.get_center()).normalize() * INITIAL_BALL_SPEED)
                self.state = GameState.PLAYING
