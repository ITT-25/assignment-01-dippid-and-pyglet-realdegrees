        ...
        for i in range(len(self.objects)):
            ...
            for j in range(i + 1, len(self.objects)):
                ...
                if is_colliding and not was_colliding:
                    print(f"Collision detected between {obj1.__class__.__name__} and {obj2.__class__.__name__}")
                    ...
                    print(f"Collision ended between {obj1.__class__.__name__} and {obj2.__class__.__name__}")
    ...
    def check_collision(self, obj1: GameObject, obj2: GameObject) -> bool:
        ...
        for corner_index in range(4):
            ...
            for step in range(COLLISION_ACCURACY + 1):
                ...
                interpolated_x = prev_corner_x + (curr_corner_x - prev_corner_x) * interpolation_factor
                interpolated_y = prev_corner_y + (curr_corner_y - prev_corner_y) * interpolation_factor
                if (interpolated_x, interpolated_y) in obj2:
                    return True
        return False

    def check_out_of_bounds(self, obj: GameObject) -> bool:
        return (obj.shape.x + obj.shape.width < 0 or
                obj.shape.x > self.window.width or
                obj.shape.y + obj.shape.height < 0 or
                obj.shape.y > self.window.height)
